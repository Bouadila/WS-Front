...
function DashboardContent() {
  ...
  React.useEffect(() => {
    (() => {
      fetch('http://localhost:8080/Transport/vehicule')
  ...
  const orederByPoids = () => {
    fetch(ordred ? 'http://localhost:8080/Transport/vehicule' : 'http://localhost:8080/Transport/vehicule/iveco')
      .then((response) => response.json())
      .then((data) => {
        setData(data.results.bindings);
        if (filtred)
  ...
  const filterByPoids = () => {
    fetch(!filtred ? 'http://localhost:8080/Transport/vehicule/ordred' : 'http://localhost:8080/Transport/vehicule')
      .then((response) => response.json())
      .then((data) => {
        console.log({data})
        setData(data.results.bindings);
        ...
        <Grid container spacing={3}>
          ...
          <Grid item xs={12}>
            <Paper sx={{ p: 2, display: "flex", flexDirection: "column" }}>
              ...
              <Button onClick={orederByPoids}>{ordred ? "revert" : "iveco"}</Button>
              ...
              <Table size="small">
                ...
                <TableBody>
                  {data?.map((row) => (
                    <TableRow key={row.Vehicule.value}>
